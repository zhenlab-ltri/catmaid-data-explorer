...
// type CatmaidConnector = []

// interface CatmaidStack {
//   id: number;
//   title: string;
// }

// interface CatmaidProject {
//   id: number;
//   title: string;
//   stacks: CatmaidStack[];
// }

// interface CatmaidConnector {
...
const exportGapJunctions = async (projectId: number) => {
  ...
  connectors.connectors.forEach(c => {
    const [id, x, y, z ] = c;
    catmaidConnectorLinkMap[id] = {
      x,
      y,
      z,
      link: `${CATMAID_URL}/?pid=${projectInfo.id}&zp=${z}&yp=${y}&xp=${x}&tool=tracingtool&sid0=${projectInfo.stacks[0].id}&s0=0`
    };
  });
  ...
  return Object.entries(connectors.partners).map(([id, partners]) => {
    ...
    const { x, y, z, link } = catmaidConnectorLinkMap[id];
